...
def collect_all_records_by_device(snapshot, device_id, path="", records=None):
    ...
    if isinstance(snapshot, dict):
        for k, v in snapshot.items():
            ...
            # Recursive search
            if isinstance(v, (dict, list)):
                collect_all_records_by_device(v, device_id, records=records)
    elif isinstance(snapshot, list):
        for v in snapshot:
            if isinstance(v, dict):
                device_fields = ["device", "deviceId", "device_id", "DeviceID", "DeviceId", "imei", "id"]
    ...
    # Remove duplicates using JSON string comparison
    unique_records = []
    seen_strings = set()
    for rec in records:
    ...
    if text == "📋 My URLs":
        ...
        if is_admin:
            help_msg = (
                ...
                "• /list - Your URLs\n"
                "• /find <id> - Search device (first 3 results)\n"
...
def handle_update(u):
    ...
    if lower_text == "/start":
        welcome_msg = (
            ...
            "• Device blocking\n"
            "• Search records by device ID (First 3 results)\n"
    ...
    if lower_text.startswith("/finda"):
        parts = text.split(maxsplit=1)
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /finda device_id\n\nOr use the 📄 Export All Data button\n\nThis will export ALL records for that device as a text file.", keyboard=get_main_keyboard(is_admin))
            return
        ...
        if file_content is None:
            ...
            send_document(chat_id, file_content, filename, f"📄 Export for device: {device}")
            send_msg(chat_id, f"✅ Export complete! File sent above.", keyboard=get_main_keyboard(is_admin))
        return
    
    # /find - Show first 3 results
    if lower_text.startswith("/find"):
        ...
        if results:
            send_msg(chat_id, f"🔍 Found {len(results)} unique record(s) for device: <code>{device}</code>\n\nShowing first 3:", keyboard=get_main_keyboard(is_admin))
            for idx, (rec, url) in enumerate(results[:3], 1):
                formatted = format_device_record(rec, url, idx)
                send_msg(chat_id, formatted, keyboard=get_main_keyboard(is_admin))
            if len(results) > 3:
                send_msg(chat_id, f"📌 Found {len(results)} total records. Use /finda {device} to export all data.", keyboard=get_main_keyboard(is_admin))
        else:
            send_msg(chat_id, f"🔍 No records found for device: <code>{device}</code>", keyboard=get_main_keyboard(is_admin))
        return
    
    # Firebase URL - ONLY for user's own URLs, NOT default
