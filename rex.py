    ...
    if isinstance(snapshot, dict):
        for k, v in snapshot.items():
            ...
            if isinstance(v, dict):
                did = v.get("deviceId") or v.get("device_id") or v.get("DeviceID") or v.get("DeviceId")
                if did and str(did) == str(device_id):
                    matches.append(v)
            if isinstance(v, (dict, list)):
                matches += search_records_by_device(v, device_id)
    elif isinstance(snapshot, list):
        for v in snapshot:
            if isinstance(v, dict):
                did = v.get("deviceId") or v.get("device_id") or v.get("DeviceID") or v.get("DeviceId")
...
def safe_format_device_record(rec):
...
def manual_refresh_cache(chat_id, firebase_url=None):
    if firebase_url:
        if firebase_url == DEFAULT_FIREBASE_URL and default_firebase_active:
...
def request_permission(chat_id, firebase_url):
    ...
    if user_firebase_count[chat_id] >= MAX_FIREBASE_PER_USER:
        ...
        message = (
            ...
            f"Reply with /stop to remove one"
        )
        ...
        if data.startswith("stop_one:"):
            ...
            if user_id in pending_permissions:
                ...
                for i, url in enumerate(current_urls):
                    keyboard.append([{"text": f"Stop {i+1}", "callback_data": f"confirm_stop:{user_id}:{i}"}])
                keyboard.append([{"text": "❌ Cancel", "callback_data": f"cancel_permission:{user_id}"}])
                send_msg(chat_id, f"Select which to stop:\n<code>{firebase_url}</code>", reply_markup={"inline_keyboard": keyboard})
...
def handle_update(u):
    ...
    if lower_text == "/list":
        ...
        if not urls:
            send_msg(chat_id, "ℹ️ No active URLs")
        else:
            text = f"📋 Your URLs ({len(urls)}/{MAX_FIREBASE_PER_USER}):\n\n"
    ...
    if is_owner(chat_id):
        if lower_text == "/default_start":
            if start_default_firebase():
                send_msg(chat_id, "✅ Default started")
            else:
                send_msg(chat_id, "❌ Failed")
            return
        if lower_text == "/default_stop":
            if stop_default_firebase():
                send_msg(chat_id, "✅ Default stopped")
            else:
                send_msg(chat_id, "ℹ️ Not active")
            return
        if lower_text == "/default_refresh":
            send_msg(chat_id, refresh_default_firebase())
            return
    ...
    if lower_text.startswith("/block"):
        ...
        if reply and reply.get("text"):
            ...
            if device:
                block_device(device)
                send_msg(chat_id, f"✅ Blocked: <code>{device}</code>")
                return
        parts = text.split()
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /block device_id or reply to message")
            return
        block_device(parts[1])
        send_msg(chat_id, f"✅ Blocked: <code>{parts[1]}</code>")
        return
    ...
    if lower_text.startswith("/unblock"):
        ...
        parts = text.split()
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /unblock device_id")
            return
        if unblock_device(parts[1]):
            send_msg(chat_id, f"✅ Unblocked: <code>{parts[1]}</code>")
        else:
            send_msg(chat_id, f"ℹ️ Not blocked")
        return
    ...
    if lower_text == "/blockedlist":
        ...
        if not devices:
            send_msg(chat_id, "📭 No blocked devices")
        else:
    ...
    if lower_text == "/stopall":
        ...
        send_msg(chat_id, f"🛑 Stopped {count} users")
        return
    ...
    if lower_text.startswith("/approve"):
        ...
        parts = text.split()
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /approve user_id")
            return
        try:
            target = int(parts[1])
            approved_users.add(target)
            send_msg(chat_id, f"✅ Approved: <code>{target}</code>")
    ...
    if lower_text.startswith("/unapprove"):
        ...
        parts = text.split()
        if len(parts) < 2:
            send_msg(chat_id, "Usage: /unapprove user_id")
            return
        try:
            target = int(parts[1])
            if target in OWNER_IDS:
                send_msg(chat_id, "❌ Cannot unapprove owner")
                return
            if target in approved_users:
                approved_users.remove(target)
                stop_watcher_single(target)
                send_msg(chat_id, f"🚫 Unapproved: <code>{target}</code>")
                send_msg(target, "❌ Access revoked")
            else:
                send_msg(chat_id, f"ℹ️ Not approved")
        except:
            send_msg(chat_id, "❌ Invalid ID")
        return
    ...
    if lower_text == "/approvedlist":
        ...
        if not approved_users:
            send_msg(chat_id, "No approved users")
            return
        lines = []
        ...
        for url in urls:
            ...
            if matches:
                ...
                for rec in matches[:2]:
                    send_msg(chat_id, f"🔍 In <code>{url}</code>\n\n{safe_format_device_record(rec)}")
        if not found:
            send_msg(chat_id, "🔍 Not found")
        return
    ...
    if text.startswith("http"):
        if text in used_firebase_urls and text not in firebase_urls.get(chat_id, []):
            ...
            if not admin_using and not is_owner(chat_id):
                send_msg(chat_id, "❌ URL already in use by another user")
                return
        test_url = normalize_json_url(text)
        if not http_get_json(test_url):
            send_msg(chat_id, "❌ Cannot fetch URL. Make sure it's public")
            return
    ...
    if lower_text == "/help":
        if is_owner(chat_id):
            help_msg = (
                "📚 <b>Full Help</b>\n\n"
                "👤 User:\n/start, /stop, /list, /find, /ping, /refresh, /help\n\n"
                "👑 Admin:\n/adminlist, /approve, /unapprove, /approvedlist\n/block, /unblock, /blockedlist\n/stopall, /broadcast, /stats\n/default_start, /default_stop, /default_refresh, /default_status\n\n"
    ...
    # Unknown
    send_msg(chat_id, "❓ Unknown command. Send /help")
